...
class ModuleGenerationContext(
    ...
    // Built once per module generation - it walks every module in the group
    val modernTypeScriptTransformer: ModernTypeScriptTransformer by lazy {
        ModernTypeScriptTransformer(profile, domain.queries, typesWorldApi)
    }
}
    ...
    : ModulePartGenerator()
{
    ...
    private fun generatePatternFile(initContent: FileContent, fileName: String = patternName().name): GeneratedPattern {
        var finalContent = toModernTypeScriptIfNeeded(initContent)
    ...
    private fun toModernTypeScriptIfNeeded(content: FileContent): FileContent {
        ...
        return c.modernTypeScriptTransformer.transform(
            content,
            ModernTypeScriptFile(c.module.getName(), submodule, patternName())
        )
    }
