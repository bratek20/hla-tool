...
class ModuleGroupParserLogic(
    ...
    private fun parseModuleFile(file: File): ModuleDefinition {
        ...
        return ModuleDefinition.create(
            ...
            webSubmodule = parseWebSubmodule(elements, moduleName),
    ...
    private fun parseWebSubmodule(elements: List<ParsedElement>, moduleName: ModuleName): WebSubmoduleDefinition? {
        return findSection(elements, "Web")?.let { web ->
            ...
            val playFabHandlers = findSection(web.elements, "PlayFabHandlers")?.let { s ->
                PlayFabHandlersDefinition(
                    exposedInterfaces = parseExposedInterfaces(s.elements, moduleName),
    ...
    private fun parseExposedInterfaces(elements: List<ParsedElement>, moduleName: ModuleName): List<ExposedInterface> {
        return findSection(elements, "ExposedInterfaces")?.elements?.filterIsInstance<Section>()?.map { section ->
            ...
            val exposedName = moduleName.value + if (isDebug) ".Debug" else ""
            ExposedInterface(
                name = section.name,
