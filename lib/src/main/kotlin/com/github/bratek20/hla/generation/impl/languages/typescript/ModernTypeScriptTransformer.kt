...
private val MEMBER_QUALIFIERS = mapOf(
    "Impl" to (SubmoduleName.Impl to PatternName.Logic),
...
class ModernTypeScriptTransformer(
    ...
    private val exports = ModernTypeScriptExports(queries)
    private val moduleNames = queries.modules.map { it.getName() }.toSet()
    ...
    private fun resolveQualified(
        ...
        MEMBER_QUALIFIERS[qualifier]?.let { (submodule, pattern) ->
        ...
        if (qualifier == SubmoduleName.Web.name) {
            if (member.isEmpty()) return null
            val file = exports.findInModule(member, module) ?: return null
