...
class ModernTypeScriptTransformer(
    ...
    // Only modules of the current group are generated by this modern profile. Modules
    // pulled in through `imports` belong to another group with its own profile - for the
    // legacy profiles that is namespaces and ambient globals, so their symbols must keep
    // their `Module.X.y` form and never get an ES import. One modern group importing
    // another is not resolved either: that would need paths relative to the other project.
    private val modernModules = queries.group.getModules()
