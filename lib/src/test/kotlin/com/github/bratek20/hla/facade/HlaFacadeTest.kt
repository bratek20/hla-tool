...
//        )
    ...
    @Test
    fun `should skip module with mismatched TypeScript modern flag on updateAllModules for legacy profile`() {
        //given
        val (directoriesMock, facade, _, loggerMock) = setup()

        // hla2 only has ImportingModule, marked "TypeScript { modern = true }" - it must not be
        // legacy-generated by the non-modern "typeScript2" profile
        val args = AllModulesOperationArgs.create(
            profileName = ProfileName("typeScript2"),
            hlaFolderPath = Path("../example/hla2"),
        )
        ...
        //then
        directoriesMock.assertWriteCount(0)
        assertThat(loggerMock.getInfos()).contains(
            "Skipping module ImportingModule for profile typeScript2 - TypeScript modern flag mismatch"
        )
    }

    @Test
    fun `should generate module with matching TypeScript modern flag on updateAllModules for modern profile`() {
        //given
        val (directoriesMock, facade, _, loggerMock) = setup()

        val args = AllModulesOperationArgs.create(
            profileName = ProfileName("typeScript2Modern"),
            hlaFolderPath = Path("../example/hla2"),
        )
        ...
        //then
        directoriesMock.assertWriteCount(2)
        assertThat(loggerMock.getInfos()).noneMatch {
            it.contains("Skipping module")
        }
    }
